...
try:
    ...
    raise InvalidCacheBackendError(
        "QDB cache backend requires the 'qdb-py' library")
from qdb.connection import UnixDomainSocketConnection, Connection
from qdb.connection import DefaultParser
...
class CacheConnectionPool(object):
    ...
    def get_connection_pool(self, host='127.0.0.1', port=6380, db=1,
                            parser_class=None, unix_socket_path=None):
        ...
        if not self._connection_pools.get(connection_identifier):
            ...
            self._connection_pools[connection_identifier] = qdb.ConnectionPool(**kwargs)
...
class CacheClass(BaseCache):
    def __init__(self, server, params):
        """
        Connect to QDB, and set up cache backend.
        """
    ...
    @property
    def server(self):
        return self._server or "127.0.0.1:6380"
    ...
    def clear(self):
        """
        Flush all cache keys. QDB do not support flushdb command.
        """
    ...
    def set_many(self, data, timeout=None, version=None):
        """
        Set a bunch of values in the cache by calling set method multiple times.
    ...
    def incr(self, key, delta=1, version=None):
        ...
        except qdb.ResponseError:
            value = self.get(key) + 1
            self.set(key, value)
        return value


class QdbCache(CacheClass):
    """
